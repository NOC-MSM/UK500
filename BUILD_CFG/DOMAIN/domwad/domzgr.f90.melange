...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namzgr_sco in configuration namelist', lwp )
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            ...
            IF( mig(ji) <= 9   .or. mjg(jj) <= 9 .or.  mjg(jj) >=jpjglo-9) then
      ...
      !CEODTEMP      where (bathy (:,:) .lt. 0.5)  bathy(:,:) = 0.0 !added clearance for min depth values  0.5 never going to need more that 50cm
      where (bathy (:,:) .lt. 0.5)  bathy(:,:) = 0.0 !added clearance for min depth values  0.5 never going to need more that 50cm
      !CEOD FOR EXPLICIT10M      where (bathy (:,:) .lt. 0.0)  bathy(:,:) = 0.0 !added clearance for min depth values  0.5 never going to need more that 50cm
      !First step excludes crazy points
      !      where (bathy (:,:) .lt.  20.0 .and. bathy(:,:) >0.0)   bathy(:,:) = 20.0 !added clearance for min depth values  0.5 never going to need more that 50cm
